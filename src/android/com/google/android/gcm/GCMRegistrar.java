 ...
 * <p>
 * <strong>Note:</strong> this class uses a private {@link SharedPreferences} object to keep track of the registration token.
 */
public final class GCMRegistrar {
    ...
    private static final String PROPERTY_SENDER_IDS = "senderIds";
     ...
     * <li>The {@link BroadcastReceiver}(s) uses the {@value com.google.android.gcm.GCMConstants#PERMISSION_GCM_INTENTS} permission.
     ...
     * <p>
     * This method should be used during development time to verify that the manifest is properly set up, but it doesn't need to be
     * called once the application is deployed to the users' devices.
     *
     * @param context
     *            application context.
    ...
    public static void register(Context context, String... senderIds) {
        ...
        Set<String> tempSenderIds = prefs.getStringSet(PROPERTY_SENDER_IDS, new HashSet<String>());
        if (senderIds != null) {
            boolean changed = false;
            for (String senderId : senderIds) {
                changed |= tempSenderIds.add(senderId);
            }
            if (changed) {
                Editor editor = prefs.edit();
                editor.putStringSet(PROPERTY_SENDER_IDS, tempSenderIds);
                editor.commit();
            }
        }
        ...
        if (sRetryReceiver == null) {
            if (sRetryReceiverClassName == null) {
                ...
                try {
                    ...
                    Log.e(TAG,
                            "Could not create instance of " + sRetryReceiverClassName + ". Using "
     ...
     * 
     * @return the array of registered sender IDs.
     */
    public static String[] getSenderIds(Context context) {
        SharedPreferences prefs = getGCMPreferences(context);
        return prefs.getStringSet(PROPERTY_SENDER_IDS, new HashSet<String>()).toArray(new String[0]);
    }
    ...
    public static String getRegistrationId(Context context) {
        ...
        // check if app was updated; if so, it must clear registration id to avoid a race condition if GCM sends a message
     ...
     *
     * <p>
     * To avoid the scenario where the device sends the registration to the server but the server loses it, this flag has an expiration
     * date, which is {@link #DEFAULT_ON_SERVER_LIFESPAN_MS} by default (but can be changed by
     ...
     *
     * @return value set by {@link #setRegisteredOnServer(Context, boolean)} or {@link #DEFAULT_ON_SERVER_LIFESPAN_MS} if not set.
     */
